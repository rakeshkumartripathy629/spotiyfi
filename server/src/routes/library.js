...
router.post('/favorites/:trackId', async (req, res) => {
  ...
  await User.findByIdAndUpdate(req.authId, { $pull: { favorites: { id: String(track.id) } } })
...
router.get('/playlists', async (req, res) => {
  const playlists = await Playlist.find({ user: req.authId }).sort({ updatedAt: -1 })
  res.json({ playlists })
})
...
router.post('/playlists', async (req, res) => {
  ...
  const playlist = await Playlist.create({ name: name.trim(), user: req.authId, tracks: [] })
  res.status(201).json({ playlist })
})

router.get('/playlists/:id', async (req, res) => {
  const playlist = await Playlist.findOne({ _id: req.params.id, user: req.authId })
...
router.delete('/playlists/:id', async (req, res) => {
  const playlist = await Playlist.findOneAndDelete({ _id: req.params.id, user: req.authId })
  if (!playlist) return res.status(404).json({ error: 'Playlist not found' })
  res.json({ ok: true })
})
